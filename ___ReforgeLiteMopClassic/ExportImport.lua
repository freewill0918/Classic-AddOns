...
function ReforgeLiteMopClassic:DisplayMessage(message, name, copyOnly)
...
function ReforgeLiteMopClassic:DebugMethod()
...
function ReforgeLiteMopClassic:PrintLog()
    self:DisplayMessage(table.concat(addonTable.printLog, "\n"), "Print Log")
end

function ReforgeLiteMopClassic:ExportJSON(preset, name)
    self:DisplayMessage(C_EncodingUtil.SerializeJSON(preset), name, true)
end

function ReforgeLiteMopClassic:ImportData(anchor)
    self:Initialize()
    self:UpdateItems()
    ...
    local function ParseUserInput(widget)
        ...
        elseif type(wowsims) == "table" and wowsims.slot then
            local function UpdateText(item)
                if _G[FRAME_NAME] then
                    _G[FRAME_NAME]:SetStatusText(L["%s does not match your currently equipped %s: %s. ReforgeLiteMopClassic only supports equipped items."]:format(
