...
TargetThreatText:SetPoint("RIGHT", TargetFrameNameBackground, "LEFT", -5, 24); -- 仇恨百分比的位置
...
TargetFrameTextureFrameHealthBarText:SetFont("Font\\bHEI00M.ttf", 12, "THICKOUTLINE");
...
TargetFrameTextureFrameManaBarText:SetFont("Font\\bHEI00M.ttf", 12, "THICKOUTLINE");
        ...
        if (buffName) then
            ...
            if icon then
                if (not _G[frameName.."CooldownText"]) then
                    ...
                    BuffCooldownText:SetFont(STANDARD_TEXT_FONT, 12, "THICKOUTLINE");
            ...
            if ( TargetFrame_ShouldShowDebuffs("target", caster, nameplateShowAll, casterIsPlayer) ) then
                ...
                if ( icon ) then
                    if (not _G[frameName.."CooldownText"]) then
                        ...
                        DebuffCooldownText:SetFont(STANDARD_TEXT_FONT, 12, "THICKOUTLINE");
        ...
        if UnitFramesPlusDB["target"]["portraittype"] == 1 then
            ...
            tpt:SetScript("OnEvent", function(self, event, ...)
                ...
                elseif event == "UNIT_HEALTH_FREQUENT" then
                    ...
                    local lightValues = { omnidirectional = false, point = CreateVector3D(0, 0, 0), ambientIntensity = 1.0};
					if (not UnitIsConnected("target")) or UnitIsGhost("target") then
                        lightValues.ambientColor = CreateColor(0.25, 0.25, 0.25)
						Target3DPortrait:SetLight(true, lightValues);
                    elseif UnitIsDead("target") then
                        lightValues.ambientColor = CreateColor(1, 0.3, 0.3)
						Target3DPortrait:SetLight(true, lightValues);
                    else
                        lightValues.ambientColor = CreateColor(1, 1, 1)
						Target3DPortrait:SetLight(true, lightValues);
                    end
                end
            end)
...
local function UnitFramesPlus_SysToT()
    if UnitFramesPlusDB["target"]["systot"] == 1 then
        if tonumber(C_CVar.GetCVar("showTargetOfTarget")) == 1 then
...
function UnitFramesPlus_TargetExtraTextFontSize()
    UFP_TargetThreatText:SetFont(STANDARD_TEXT_FONT, UnitFramesPlusDB["target"]["fontsize"], "THICKOUTLINE");
    UFP_TargetHPText:SetFont(TextStatusBarText:GetFont(), UnitFramesPlusDB["target"]["fontsize"], "THICKOUTLINE");
    UFP_TargetMPText:SetFont(TextStatusBarText:GetFont(), UnitFramesPlusDB["target"]["fontsize"], "THICKOUTLINE")
    UFP_TargetHPMPPctHP:SetFont(STANDARD_TEXT_FONT, UnitFramesPlusDB["target"]["fontsize"], "THICKOUTLINE");
    UFP_TargetHPMPPctMP:SetFont(STANDARD_TEXT_FONT, UnitFramesPlusDB["target"]["fontsize"], "THICKOUTLINE");
    UFP_TargetHPMPPctPct:SetFont(STANDARD_TEXT_FONT, UnitFramesPlusDB["target"]["fontsize"], "THICKOUTLINE");
    UFP_TargetRace:SetFont(STANDARD_TEXT_FONT, UnitFramesPlusDB["target"]["fontsize"], "THICKOUTLINE");

    if (not isEasyFramesLoaded) then
...
function UnitFramesPlus_TargetInit()
	...
	if IsAddOnLoaded("EasyFrames") then
		UnitFramesPlusDB["target"]["bartext"] = 0
