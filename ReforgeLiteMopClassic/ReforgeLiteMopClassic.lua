...
local ReforgeLiteMopClassic = CreateFrame("Frame", addonName, UIParent, "BackdropTemplate")
addonTable.ReforgeLiteMopClassic = ReforgeLiteMopClassic
...
ReforgeLiteMopClassic.itemSlots = ITEM_SLOTS
...
local RFL_FRAMES = { ReforgeLiteMopClassic }
...
ReforgeLiteMopClassic.playerData = PLAYER_ITEM_DATA

addonTable.localeClass, addonTable.playerClass, addonTable.playerClassID = UnitClass("player")
local UNFORGE_INDEX = -1
addonTable.StatCapMethods = EnumUtil.MakeEnum("AtLeast", "AtMost", "NewValue", L["Exactly"])

function ReforgeLiteMopClassic:UpgradeDB()
  local db = ReforgeLiteMopClassicDB
...
GUI.CreateStaticPopup("REFORGE_LITE_SAVE_PRESET", L["Enter the preset name"], { func = function(text)
  ReforgeLiteMopClassic.cdb.customPresets[text] = {
    caps = CopyTable(ReforgeLiteMopClassic.pdb.caps),
    weights = CopyTable(ReforgeLiteMopClassic.pdb.weights)
  }
  ReforgeLiteMopClassic:InitCustomPresets()
  ReforgeLiteMopClassic.deletePresetButton:ToggleStatus()
end })

local statIds = EnumUtil.MakeEnum("SPIRIT", "DODGE", "PARRY", "HIT", "CRIT", "HASTE", "EXP", "MASTERY", "SPELLHIT")
addonTable.statIds = statIds
ReforgeLiteMopClassic.STATS = statIds

local FIRE_SPIRIT = 4
local function GetFireSpirit()
  local s2h = (ReforgeLiteMopClassic.conversion[statIds.SPIRIT] or {})[statIds.HIT]
...
local StatAdditives = {
  ...
  [CR_MASTERY] = function(rating)
    if ReforgeLiteMopClassic.pdb.mastery and not ReforgeLiteMopClassic:PlayerHasMasteryBuff() then
...
local ITEM_STATS = {
    ...
    --RatingStat (statIds.HIT,     "ITEM_MOD_HIT_RATING",           HIT,            HIT,                  CR_HIT),
    {
      ...
      getter = function()
        ...
        if (ReforgeLiteMopClassic.conversion[statIds.EXP] or {})[statIds.HIT] then
          hit = hit + (GetCombatRating(CR_EXPERTISE) * ReforgeLiteMopClassic.conversion[statIds.EXP][statIds.HIT])
        end
        return hit
      end,
      mgetter = function (method, orig)
...
ReforgeLiteMopClassic.reforgeTable = reforgeTable

addonTable.REFORGE_COEFF = 0.4

function ReforgeLiteMopClassic:UpdateWindowSize ()
  self.db.windowWidth = self:GetWidth ()
  self.db.windowHeight = self:GetHeight ()
end

function ReforgeLiteMopClassic:GetCapScore (cap, value)
  local score = 0
...
function ReforgeLiteMopClassic:GetStatScore (stat, value)
...
function ReforgeLiteMopClassic:ValidateWoWSimsString(importStr)
...
function ReforgeLiteMopClassic:ApplyWoWSimsImport(newItems, attachToReforge)
...
function ReforgeLiteMopClassic:ParsePresetString(presetStr)
...
function ReforgeLiteMopClassic:PreviewColors()
...
function ReforgeLiteMopClassic:ValidatePawnString(importStr)
...
function ReforgeLiteMopClassic:ParsePawnString(values)
  local raw = {}
  local average = 0
  local total = 0
...
function ReforgeLiteMopClassic:CreateCategory (name)
...
function ReforgeLiteMopClassic:SetAnchor (frame_, point_, rel_, relPoint_, offsX, offsY)
...
function ReforgeLiteMopClassic:GetFrameY (frame)
  local cur = frame
  local offs = 0
...
function ReforgeLiteMopClassic:SetScroll (value)
...
function ReforgeLiteMopClassic:FixScroll ()
...
function ReforgeLiteMopClassic:SetNewTopWindow(newTopWindow)
...
function ReforgeLiteMopClassic:CreateFrame()
  self:InitPresets()
  ...
  self.scrollBar = CreateFrame ("Slider", "ReforgeLiteMopClassicScrollBar", self.scrollFrame, "UIPanelScrollBarTemplate")
...
function ReforgeLiteMopClassic:CreateItemTable ()
  ...
  GUI:SetTooltip(self.itemLockHelpButton, L["The current state of your equipment.\nClicking an item icon will lock it. ReforgeLiteMopClassic will ignore the item(s) in future calculations."])
...
function ReforgeLiteMopClassic:AddCapPoint (i, loading)
  ...
  local methodList = {
    {value = addonTable.StatCapMethods.AtLeast, name = L[L["At least"]]},
...
function ReforgeLiteMopClassic:RemoveCapPoint (i, point, loading)
...
function ReforgeLiteMopClassic:ReorderCapPoint (i, point)
  local newpos = point
...
function ReforgeLiteMopClassic:UpdateCapPreset (i, point)
...
function ReforgeLiteMopClassic:UpdateCapPoints (i)
...
function ReforgeLiteMopClassic:RefreshCaps()
...
function ReforgeLiteMopClassic:CollapseStatCaps()
...
function ReforgeLiteMopClassic:SetStatWeights (weights, caps)
  if weights then
...
function ReforgeLiteMopClassic:CapUpdater ()
...
function ReforgeLiteMopClassic:UpdateStatWeightList ()
...
function ReforgeLiteMopClassic:CreateOptionList ()
  ...
  self.savePresetButton = GUI:CreatePanelButton (self.content, SAVE, function() StaticPopup_Exibir ("REFORGE_LITE_SAVE_PRESET") end)
  ...
  local buffsContextValues = {
    spellHaste = { text = addonTable.CreateIconMarkup(136092) .. L["Spell Aceleração"], selected = self.PlayerHasSpellHasteBuff },
    meleeHaste = { text = addonTable.CreateIconMarkup(133076) .. L["Melee Aceleração"], selected = self.PlayerHasMeleeHasteBuff },
...
function ReforgeLiteMopClassic:GetActiveWindow()
...
function ReforgeLiteMopClassic:GetInactiveWindows()
  if(not RFL_FRAMES[2]) then
    return {}
  end
...
function ReforgeLiteMopClassic:FillSettings()
  ...
  accuracySlider.Text:SetText (L[L["Accuracy"]])

  GUI:SetTooltip(accuracySlider, L["Setting to Baixo will result in lower accuracy but faster results! Set this back to Alto if you're not getting the results you expect."])

  self.settings:SetCell (getOrderId('settings'), 0, accuracySlider, "LEFT", 8)

  self.settings:SetCell (getOrderId('settings'), 0, GUI:CreateCheckButton (self.settings, L[L["Open window when reforging"]],
    self.db.openOnReforge, function (val) self.db.openOnReforge = val end), "LEFT")

  self.settings:SetCell (getOrderId('settings'), 0, GUI:CreateCheckButton (self.settings, L[L["Summarize reforged stats on tooltip"]],
  ...
  self.settings:SetCell (getOrderId('settings'), 0, GUI:CreateCheckButton (self.settings, L[L["Enable spec profiles"]],
  ...
  self.settings:SetCell (getOrderId('settings'), 0, GUI:CreateCheckButton (self.settings, L["Exibir import button on Reforging window"],
...
function ReforgeLiteMopClassic:CreateImportButton()
...
function ReforgeLiteMopClassic:UpdateMethodCategory()
...
function ReforgeLiteMopClassic:RefreshMethodStats()
  if self.pdb.method then
...
function ReforgeLiteMopClassic:UpdateContentSize ()
  self.content:SetHeight (-self:GetFrameY (self.lastElement))
  RunNextFrame(function() self:FixScroll() end)
end

function ReforgeLiteMopClassic:GetReforgeTableIndex(src, dst)
...
function ReforgeLiteMopClassic:UpdateItems()
...
function ReforgeLiteMopClassic:UpdatePlayerSpecInfo()
...
function ReforgeLiteMopClassic:RegisterQueueUpdateEvents()
...
function ReforgeLiteMopClassic:UnregisterQueueUpdateEvents()
...
function ReforgeLiteMopClassic:QueueUpdate()
  local time = GetTime()
...
function ReforgeLiteMopClassic:CreateMethodWindow()
  self.methodWindow = CreateFrame ("Frame", "ReforgeLiteMopClassicMethodWindow", UIParent, "BackdropTemplate")
  ...
  self.methodWindow.cost = CreateFrame("Frame", "ReforgeLiteMopClassicReforgeCost", self.methodWindow, "SmallMoneyFrameTemplate")
...
function ReforgeLiteMopClassic:RefreshMethodWindow()
...
function ReforgeLiteMopClassic:ShowMethodWindow(attachToReforge)
  if not self.methodWindow then
...
function ReforgeLiteMopClassic:UpdateMethodChecks ()
...
function ReforgeLiteMopClassic:SwapSpecProfiles()
...
function ReforgeLiteMopClassic:DoReforge()
...
function ReforgeLiteMopClassic:StopReforging()
  if reforgeCo then
    reforgeCo = nil
    ClearReforgeWindow()
    collectgarbage()
...
function ReforgeLiteMopClassic:ContinueReforge()
...
function ReforgeLiteMopClassic:DoReforgeUpdate()
  if self.methodWindow then
...
local function HandleTooltipUpdate(tip)
  if not ReforgeLiteMopClassic.db.updateTooltip then return end
...
function ReforgeLiteMopClassic:HookTooltipScripts()
...
function ReforgeLiteMopClassic:OnEvent(event, ...)
  if self[event] then
    self[event](self, ...)
  end
...
function ReforgeLiteMopClassic:Initialize()
...
function ReforgeLiteMopClassic:OnShow()
  self:Initialize()
  self:SetNewTopWindow()
  self:UpdateItems()
  self:RegisterQueueUpdateEvents()
...
function ReforgeLiteMopClassic:OnHide()
  local activeWindow = self:GetActiveWindow()
...
function ReforgeLiteMopClassic:OnCommand (cmd)
  if InCombatLockdown() then print(ERROR_CAPS, ERR_AFFECTING_COMBAT) return end
  self:Show()
end

function ReforgeLiteMopClassic:FORGE_MASTER_ITEM_CHANGED()
  self:ContinueReforge()
end

function ReforgeLiteMopClassic:FORGE_MASTER_OPENED()
...
function ReforgeLiteMopClassic:FORGE_MASTER_CLOSED()
...
function ReforgeLiteMopClassic:PLAYER_REGEN_DISABLED()
  RFL_FRAMES:CloseAll()
end

local currentSpec -- hack because this event likes to fire twice
function ReforgeLiteMopClassic:ACTIVE_TALENT_GROUP_CHANGED(curr)
...
function ReforgeLiteMopClassic:PLAYER_SPECIALIZATION_CHANGED()
  self:GetConversion()
  self:UpdatePlayerSpecInfo()
end

function ReforgeLiteMopClassic:PLAYER_ENTERING_WORLD()
  self:GetConversion()
end

function ReforgeLiteMopClassic:PLAYER_AVG_ITEM_LEVEL_UPDATE()
  self.itemLevel:SetFormattedText(CHARACTER_LINK_ITEM_LEVEL_TOOLTIP, select(2,GetAverageItemLevel()))
end

function ReforgeLiteMopClassic:ADDON_LOADED (addon)
...
ReforgeLiteMopClassic:SetScript ("OnEvent", ReforgeLiteMopClassic.OnEvent)
ReforgeLiteMopClassic:RegisterEvent ("ADDON_LOADED")




-- Fix CloseWindows / UIPanelWindows:
-- Some code registers "ReforgeLiteMopClassic" in UISpecialFrames/UIPanelWindows.
-- Blizzard then does _G["ReforgeLiteMopClassic"]:IsShown().
-- In this renamed addon, _G["ReforgeLiteMopClassic"] is the addon table, not the real frame.
-- These wrapper methods prevent the nil IsShown error and forward calls to the real frame.
do
    ...
    if type(addonTable) == "table" and type(addonTable.IsShown) ~= "function" then
        ...
        function addonTable:SetShown(state)
            if state then
                return self:Show()
            else
                return self:Hide()
