...
function ExtVendor_DoQuickVendorRefresh()
    ...
    while true do
        ...
        if (REFRESH_CURRENT_SLOT <= REFRESH_SLOTS_IN_BAG) then
            ...
            cInfo = C_Container.GetContainerItemInfo(REFRESH_CURRENT_BAG, REFRESH_CURRENT_SLOT);
...
function ExtVendor_ProgressQuickVendor()
    ...
    local link, count, name, color, quality, itemLevel, itemReqLevel, price, maxStack, quantity, bindType, expacID, itemType, itemSubType, itemEquipLoc, itemClassId, itemSubClassId, __, cInfo, locked;
    local isKnown, reqClasses, itemId, isAccountBound, isFoodOrDrink;
    local bag, slot;
    local totalPrice = 0;
    local itemsOnLine = 0;
        ...
        if (C_Container.GetContainerNumSlots(bag)) then
            for slot = 1, C_Container.GetContainerNumSlots(bag), 1 do
                ...
                if (not CANCEL) then
                    ...
                    -- __, count, locked = C_Container.GetContainerItemInfo(bag, slot);
                    cInfo = C_Container.GetContainerItemInfo(bag, slot);
