...
function ZygorTalentAdvisorPopout_OnShow(self)
	...
	TalentFrame_LoadUI()
	if ZGV.db.profile.zta_windowdocked then
...
function ZygorTalentAdvisorPopout_OnDragStop(self)
	...
	then
		ZGV.db.profile.zta_windowdocked = true
	else
		ZGV.db.profile.zta_windowdocked = false
	end
	ZygorTalentAdvisorPopout_Reparent()
...
function ZygorTalentAdvisorPopout_Update()
	if not PlayerTalentFrame or not PlayerTalentFrame.advisorbutton then return end
	ZTA:Debug("ZygorTalentAdvisorPopout_Update")
	...
	if GlyphFrame and GlyphFrame:IsShown() then
		...
		PlayerTalentFrame.advisorbutton:SetPoint("TOPRIGHT", PlayerTalentFrame, "BOTTOMRIGHT", -5, 33)
	else
		PlayerTalentFrame.advisorbutton:ClearAllPoints()
		PlayerTalentFrame.advisorbutton:SetPoint("TOPRIGHT", PlayerTalentFrame, "TOPRIGHT", -5, -30)
	end
...
function ZygorTalentAdvisorPopout_UpdateDocking()
	...
	--[[
	if PlayerTalentFrame.advisorbutton then
		if ZGV.db.profile.zta_windowdocked and self:IsShown() then
