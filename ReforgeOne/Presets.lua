...
local CloneTable = addonTable.DeepCopy or CopyTable
local tInvert = _G.tInvert or function(tbl)
  local inverted = {}
...
local function LocalizedCapName(key)
  return function()
    return L[key]
  end
end
...
function ReforgeOne:PlayerHasSpellHasteBuff()
...
function ReforgeOne:PlayerHasMeleeHasteBuff()
...
function ReforgeOne:PlayerHasMasteryBuff()
...
function ReforgeOne:RatingPerPoint (stat, level)
...
function ReforgeOne:GetMeleeHitBonus ()
...
function ReforgeOne:GetSpellHitBonus ()
...
function ReforgeOne:GetExpertiseBonus()
...
function ReforgeOne:GetNonSpellHasteBonus(hasteFunc, ratingBonusId)
...
function ReforgeOne:GetMeleeHasteBonus()
...
function ReforgeOne:GetRangedHasteBonus()
...
function ReforgeOne:GetSpellHasteBonus()
...
function ReforgeOne:GetHasteBonuses()
...
function ReforgeOne:CalcHasteWithBonus(haste, hasteBonus)
...
function ReforgeOne:CalcHasteWithBonuses(haste)
...
function ReforgeOne:GetNeededMeleeHit ()
...
function ReforgeOne:GetNeededSpellHit ()
...
function ReforgeOne:GetNeededExpertiseSoft()
  return max(0, 3 + 1.5 * self.pdb.targetLevel)
end
...
function ReforgeOne:GetNeededExpertiseHard()
...
local function CreateIconMarkup(icon)
  ...
  elseif CreateSimpleTextureMarkup then
...
local MakeEnum = (EnumUtil and EnumUtil.MakeEnum) or function(...)
  local enum = {}
...
local CAPS = MakeEnum("ManualCap", "MeleeHitCap", "SpellHitCap", "MeleeDWHitCap", "ExpSoftCap", "ExpHardCap", "FirstHasteBreak", "SecondHasteBreak", "ThirdHasteBreak", "FourthHasteBreak", "FifthHasteBreak")

ReforgeOne.capPresets = {
  {
  ...
  },
  {
    ...
    name = LocalizedCapName("Melee hit cap"),
    getter = function ()
      return ReforgeOne:RatingPerPoint(StatHit) * (ReforgeOne:GetNeededMeleeHit() - ReforgeOne:GetMeleeHitBonus())
    end,
    category = StatHit
  },
  {
    value = CAPS.SpellHitCap,
    name = LocalizedCapName("Spell hit cap"),
    getter = function ()
      return ReforgeOne:RatingPerPoint (addonTable.statIds.SPELLHIT) * (ReforgeOne:GetNeededSpellHit () - ReforgeOne:GetSpellHitBonus ())
    end,
    category = StatHit
  },
  {
    value = CAPS.MeleeDWHitCap,
    name = LocalizedCapName("Melee DW hit cap"),
    getter = function ()
      return ReforgeOne:RatingPerPoint(StatHit) * (ReforgeOne:GetNeededMeleeHit() + 19 - ReforgeOne:GetMeleeHitBonus())
    end,
    category = StatHit
  },
  {
    value = CAPS.ExpSoftCap,
    name = LocalizedCapName("Expertise soft cap"),
    getter = function ()
      return ReforgeOne:RatingPerPoint (StatExp) * (ReforgeOne:GetNeededExpertiseSoft() - ReforgeOne:GetExpertiseBonus())
    end,
    category = StatExp
  },
  {
    value = CAPS.ExpHardCap,
    name = LocalizedCapName("Expertise hard cap"),
    getter = function ()
      return ReforgeOne:RatingPerPoint (StatExp) * (ReforgeOne:GetNeededExpertiseHard() - ReforgeOne:GetExpertiseBonus())
    end,
    category = StatExp
  },
}
...
local function GetSpellHasteRequired(percentNeeded)
  return function()
    local hasteMod = ReforgeOne:GetSpellHasteBonus()
    return ceil((percentNeeded - (hasteMod - 1) * 100) * ReforgeOne:RatingPerPoint(addonTable.statIds.HASTE) / hasteMod)
  end
end
...
---@return nil
function ReforgeOne:InitClassPresets()
  local specs = {
...
---@return nil
function ReforgeOne:InitCustomPresets()
  local customPresets = {}
  for k, v in pairs(self.cdb.customPresets) do
    local preset = CloneTable(v)
    preset.name = k
...
---@return nil
function ReforgeOne:InitDynamicPresets()
...
---@return nil
function ReforgeOne:InitPresets()
      ...
      GUI.CreateStaticPopup("REFORGE_LITE_SAVE_PRESET",
        ...
        function(popup)
          self.cdb.customPresets[popup:GetEditBox():GetText()] = {
            caps = CloneTable(self.pdb.caps),
            weights = CloneTable(self.pdb.weights)
          }
          self:InitCustomPresets()
        end, {
          hasEditBox = true,
          ...
          onShow = addonTable.SetupSavePresetPopup,
          onHide = addonTable.ResetSavePresetPopup,
        })
          ...
          if IsShiftKeyDown() then
            GUI.CreateStaticPopup("REFORGE_LITE_DELETE_PRESET",
              ...
              function()
                ...
                self:InitCustomPresets()
              end, { button1 = DELETE, hasEditBox = false, dialogWidth = 270 })
